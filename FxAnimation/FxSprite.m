...
-(id)init {
    ...
    self.shaderProgram = [[CCShaderCache sharedShaderCache] programForKey:@"ShaderPositionTextureTint"];
...
-(void)setSpriteFrameName:(NSString*)spf withOffsetRect:(CGRect)offsetRect{
    ...
    if([FxAnimatedSprite texturesAreHalfSized]){
        CGRect dblRect = CGRectMake(frame.rectInPixels.origin.x * 2,
                                    frame.rectInPixels.origin.y * 2,
                                    frame.rectInPixels.size.width * 2,
                                    frame.rectInPixels.size.height * 2);
        [self setContentSize:dblRect.size];
        [self setVertexRect:dblRect];
    } else {
        [self setContentSize:frame.rectInPixels.size];
        [self setVertexRect:frame.rectInPixels];
    }
...
-(void)applyAnimSprite:(FxAnimatedSprite*)sprite asLayerIndex:(NSInteger)layerIndex atFrame:(CGFloat)time wrapAround:(BOOL)wrapAround {
    ...
    if(![lastSetFrameName isEqualToString:spriteFrameName]){
        ...
        lastSetFrameName = [[spriteFrameName copy] retain];
    }
