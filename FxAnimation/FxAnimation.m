...
-(void)dealloc{
    [_name release];
    [_layerNameNumber release];
    [_layerNames release];
    [_layerSymbols release];
    [_frameLabels release];
    [_frameComments release];
    [_sequenceRanges release];
    [_animationTracks release];
    [super dealloc];
}
...
+(FxAnimation*)animationNamed:(NSString*)name {
    ...
    if(_animationCache){
        ...
        if (ret) {
            ...
            ret = [[[FxAnimation alloc] initWithName:name] autorelease];
            if(ret) {
